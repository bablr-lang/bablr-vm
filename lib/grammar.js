...
import { tokenGapTag, gapNodeTag } from './utils/ast.js';
...
const tokenAliasProductionsEnhancer = (grammar) => {
  const aliasProductions = map(([aliasType, types]) => {
    return productionFor(aliasType, function* match(props) {
      ...
      const matchables = types.map((type) => tokenGapTag(type, value, attrs));

      yield matchFrom(eat, tokenGapTag('Any', null, new Map(objectEntries({ matchables }))));
    });
  }, grammar.aliases);
