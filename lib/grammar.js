...
const setSubtypes = (productions, subtypesByType, type, value) => {
  const subtypes = subtypesByType.get(type);
  if (subtypes) {
    for (const subtype of subtypes) {
      setSubtypes(productions, subtypesByType, subtype, value);
    }
  } else {
    productions.set(type, value);
  }
};
...
const buildProductions = (productions, aliases) => {
  ...
  for (const [type, production] of productions) {
    setSubtypes(productionsByType, aliases, type, production);
  }
