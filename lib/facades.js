...
import { buildNode, isStubNode, buildPropertyTag } from '@bablr/agast-helpers/path';
      ...
      if (isObject(tag) && tag.type === Property) {
        ...
        if (reference?.flags.intrinsic || ['_', '#', '@'].includes(reference?.type)) {
          tags_ = BList.fromValues(
            [tags[1][0], tags[1][1], '', '', buildFacadeNode(reference, node)],
            1,
          );
        } else {
          let gapNode = buildNode('<//>');
          if (!reference?.flags.hasGap) {
            gapNodes.set(gapNode, node);
          }

          tags_ = BList.fromValues([tags[1][0], tags[1][1], '', '', gapNode], 1);
        }

        return buildPropertyTag(tags_, shift);
      } else {
        return tag;
      }
    }, Tags.getTags(node)),
