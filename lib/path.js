...
import { freezeSeal, isString, isFunction } from './utils/object.js';
...
export class PathFacade {
  ...
  matchParent(type, property = null) {
    const { grammar } = this[_actual];
    const { parentNode, parentProperty } = this;
    let matcher;

    if (isString(type)) {
      matcher = (node) => !(grammar.is ? grammar.is(type, node.type) : type === node.type);
    } else if (isFunction(type)) {
      matcher = type;
    } else {
      throw new Error('type argument to matchParent must be a string or function');
    }

    return (property === null ? property === parentProperty : true) && matcher(parentNode);
  }
...
export class Path {
  constructor(node, grammar, refToken = null, parent = null) {
    if (!node.type) {
