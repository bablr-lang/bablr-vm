...
export class Path extends WeakStackFrame {
  ...
  constructor(context, reference, isTerminal = false, range = [], resolver = new Resolver()) {
    super();
  ...
  get type() {
    return this.startTag.value.type;
  }

  pushTag(tag, isTerminal = false) {
    const { context } = this;

    switch (tag.type) {
      case 'Reference': {
        const path = new Path(context, tag, isTerminal);
  ...
  *parents(includeSelf = false) {
  ...
  branch() {
    const { context, reference, isTerminal, range, resolver } = this;
    const { parents, depths } = this.stack;

    const path = new Path(context, reference, isTerminal, [...range], resolver);
