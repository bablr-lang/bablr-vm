...
import { buildNode, getAttributes, getTags, TagPath } from '@bablr/agast-helpers/path';
  ...
  for (;;) {
    ...
    switch (verb) {
        ...
        switch (tag.type) {
          ...
          case GapTag: {
            ...
            if (s.held) {
              ...
              if (!s.node.value.flags.token || m.referencePath.tag.value.type === '@') {
                m.advance(buildNode(tag));
              } else {
                m.advance(buildGapTag());
              }
      ...
      case 'endFrame': {
        ...
        yield* finishedMatch.emit(options);
