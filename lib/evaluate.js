...
import { streamFromTree, treeFromStream } from '@bablr/agast-helpers/tree';
import * as Tags from '@bablr/agast-helpers/tags';

import { getEmbeddedMatcher, getEmbeddedTag } from '@bablr/agast-vm-helpers/deembed';
import { Match, buildNode } from './match.js';
import { buildEmbeddedMatcher, buildWriteEffect } from '@bablr/agast-vm-helpers/builders';
import {
  deepFreezeRecord,
  freeze,
  has,
...
import { get, getRoot, list } from '@bablr/agast-helpers/path';
...
let mergePatterns = (a, b) => {
  ...
  let a_ = isString(a)
    ? [buildAlternative(buildLiteralElements(a))]
    : list('alternatives', getRoot(a));
  let b_ = isString(b)
    ? [buildAlternative(buildLiteralElements(b))]
    : list('alternatives', getRoot(b));

  return buildEmbeddedMatcher(buildPattern(buildAlternatives([...a_, ...b_])));
};

let unwrapPattern = (pattern) => {
  return isString(pattern) ? pattern : getEmbeddedMatcher(pattern);
};
...
function* __bablr(ctx, rootSource, rootLanguage, strategy, options) {
  ...
  for (;;) {
    ...
    switch (verb) {
      ...
      case 'startSpan': {
        let {
          arguments: { 0: name, 1: guard = null, 2: merge = false, 3: { value: props } } = [],
        } = instr;
        if (!isDeepRecord(props)) throw new Error();

        if (merge) {
          let name_ = name == null ? s.span.name : `${s.span.name}:${name}`;
          let guard_ = mergePatterns(unwrapPattern(guard), unwrapPattern(s.span.guard));
          let props_ = deepFreezeRecord({ ...s.span.props, ...props });
      ...
      case 'endSpan': {
        if (BListKeyed.getSize(s.spans) <= BListKeyed.getSize(rootSpans)) throw new Error();
      ...
      case 'call': {
        ...
        if (verb === Symbol.for('shift') || verb === Symbol.for('shiftMatch')) {
          ...
          let literalMatcher = get(
            ['valueMatcher', 'nodeMatcher', 'open', 'literalValue'],
            matcher,
          );

          let literalValue = null;
          if (literalMatcher) {
            let result = yield* s.guardedMatch(buildEmbeddedMatcher(literalMatcher));
      ...
      case 'return': {
        ...
        if (finishedMatch.isNode) {
          ctx.registerNode(m.language, m.node);
        }
