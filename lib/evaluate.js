    ...
    switch (verb) {
      ...
      case 'eatHeld': {
        ...
        if (s.shifted) {
          ...
          let tag_ = finishedMatch.shiftMatch ? Tags.getAt(-1, s.held.value.tags) : s.held;
          if (finishedMatch.options.escape) {
            tag_ = Tags.getAt(-1, s.held.value.tags);
            tag_ = printTag(
          ...
          if (!finishedMatch.parentTagPath) {
            finishedMatch.parentTagPath =
              (!m.flags.token ? s.resultPath.propertyPath : s.resultPath) ||
              finishedMatch.parentPreviousTagPath;
          }
        }
