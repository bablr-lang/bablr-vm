...
import { buildNode, endsNode, getAttributes, getTags, TagPath } from '@bablr/agast-helpers/path';
    ...
    switch (verb) {
      ...
      case 'match': {
        ...
        let attributes = getAttributes(
          typeof pattern === 'string' && s.node.value.flags.token ? s.node : pattern,
        );
      ...
      case 'endFrame': {
        ...
        if (m) {
          if (m.emitted && finishedMatch.emitted) {
            let { tagPath } = m.emitted;
            if (endsNode(tagPath.tag)) {
              tagPath = tagPath.path.parentPropertyPath;
            }

            let oldTagPath = tagPath;
            if (tagPath?.nextProperty) {
              let tagPath = oldTagPath.nextProperty;

              while (
                outerOptions.holdShiftedNodes &&
                tagPath.nextProperty?.next.tag.type === ShiftTag
              ) {
                tagPath = tagPath.nextProperty;
              }

              m.emitted = {
                match: m.emitted.match,
                tagPath,
              };
            }
          }
      ...
      case 'shiftFrame': {
        ...
        let parentMatch = finishedMatch.endFrame(outerOptions);
        ...
        if (parentMatch) {
          if (parentMatch.emitted && finishedMatch.emitted) {
            let { tagPath } = parentMatch.emitted;
            if (endsNode(tagPath.tag)) {
              tagPath = tagPath.path.parentPropertyPath;
            }
            if (tagPath?.nextProperty) {
              parentMatch.emitted = {
                match: parentMatch.emitted.match,
                tagPath: tagPath.nextProperty,
              };
            }
          }
