...
import { buildNullTag, getOpenTag, treeFromStreamSync } from '@bablr/agast-helpers/tree';
  ...
  for (;;) {
    ...
    switch (verb) {
      ...
      case 'advance': {
        ...
        if (m.cover && !m.isNode) throw new Error('cannot advance inside a cover');
        ...
        switch (tag.type) {
          ...
          case ReferenceTag: {
            ...
            s.referencePath = TagPath.from(Path.from(s.node), -1);
          ...
          case GapTag: {
            if (s.source.value == null && (!s.source.done || s.source.holding)) {
              ...
              if (s.held) {
                ...
                s.resultPath = s.resultPath.nextSibling || s.resultPath;
                s.held = null;
                break;
              }
      ...
      case 'startFrame': {
        ...
        if (didShift) {
          ...
          let refTag = sumtree.getAt(-2, parentMatch.fragmentNode.children);
      ...
      case 'throw': {
        ...
        s.reject(
          rejectedMatch.fragmentNode,
          !rejectedMatch.isNode || rejectedMatch.effects.success === 'none',
          options.bind,
        );
        m = m.endFrame();
        s = m.state;

        if (ref && ['#', '@'].includes(ref.value.name) && !rejectedMatch.didShift) {
          if (shouldBranch(rejectedMatch.effects) && !hasOwn(s.node.properties, ref.value.name)) {
            if (!s.holding) {
              s.advance(ref);

              if (ref.value.isArray) {
                s.advance(buildInitializerTag(true));
              } else {
