  ...
  for (;;) {
    ...
    switch (verb) {
      ...
      case 'advance': {
        ...
        switch (tag.type) {
          ...
          case ReferenceTag: {
            ...
            s.referenceTagPath = TagPath.fromNode(s.node, -1, 0);
          ...
          case ShiftTag: {
            ...
            let refTag = s.node.tags.at(-3);
            let lastShiftTag = refTag;

            if (refTag.type === ShiftTag) {
              refTag = s.node.tags.at(-3 - refTag.value.index * 3);
            }
            ...
            m.advance(tag, s);
            s.referenceTagPath = TagPath.fromNode(s.node, 0);
            break;
          }
                    ...
                    if (tagPath.node.flags.hasGap) {
                      ...
                      vmReturn = escapeVm.next(
                        buildChild(
                          sym.Property,
                          buildProperty(
                            tagPath.path.referenceTag.value,
      ...
      case 'startFrame': {
        ...
        if (didShift) {
          if (s.node.tags.at(-1)?.type !== ShiftTag) {
