  ...
  for (;;) {
    ...
    switch (verb) {
      ...
      case 'throw': {
        ...
        let didBranch = rejectedMatch.s !== s;

        finishedState.reject(
          rejectedMatch.fragmentNode,
          (!rejectedMatch.isNode && !didBranch) || rejectedMatch.effects.success === 'none',
          // rejectedMatch.effects.success === 'none',
          options.bind,
        );
        rejectedMatch.endFrame();
