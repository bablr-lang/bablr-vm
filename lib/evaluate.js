...
import {
  ...
  buildNodeCloseTag,
} from '@bablr/ast-helpers';
import startsWithSeq from 'iter-tools-es/methods/starts-with-seq';
import { effectsFor, reifyExpression, buildNull } from '@bablr/vm-helpers';
...
export function* evaluate(ctx, rootSource, instructionGenerator) {
  let s = State.from(ctx, rootSource);

  let co = new Coroutine(instructionGenerator);
