...
import { NodeFacade, TagPathFacade as TagPath } from '@bablr/agast-vm';
  ...
  for (;;) {
    ...
    switch (verb) {
        ...
        switch (tag.type) {
          ...
          case OpenNodeTag: {
            ...
            let resultNode = literalValue
              ? m.node && new NodeFacade(m.node.tags.at(-1).value.property.node)
              : m.node;
      ...
      case 'startFrame': {
        ...
        m = Match.startFrame(language, ctx, s, m, finishedMatch, verb, matcher, options);
        s = m.state;
