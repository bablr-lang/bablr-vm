...
import { reifyExpression, buildCall, buildString } from '@bablr/agast-vm-helpers';
...
export const evaluate = (ctx, rootLanguage, rootSource, strategy) => {
  return (agastCtx, agastState) => {
    ...
    return __evaluate(ctx, rootLanguage, rootSource, agastState, strategy);
  };
};
...
const __evaluate = function* bablrStrategy(ctx, rootLanguage, rootSource, agastState, strategy) {
...

  {
    ...
    if (maybePromise instanceof Promise) {
      yield buildCall('resolve', maybePromise);
    }
  }
    ...
    switch (verb) {
      ...
      case 'match': {
        ...
        if (result instanceof Promise) {
          result = yield buildCall('resolve', result);
        }
