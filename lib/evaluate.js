...
const __evaluate = function* bablrStrategy(ctx, rootLanguage, rootSource, agastState, strategy) {
  ...
  for (;;) {
    ...
    switch (verb) {
        ...
        switch (terminal?.type || 'Null') {
          ...
          case 'OpenNodeTag': {
            ...
            const resolvedLanguage =
              oldPath.depth > 1 ? resolvedLanguages.get(oldPath.parent) : rootLanguage;
            ...
            if (balancer) {
              ...
              if (balancer && balancedPath.startTag.value.attributes.balancedSpan) {
                s.spans = s.spans.pop();
              }
            }
      ...
      case 'branch': {
        const baseState = s;
        let { context, source, agast, balanced, spans, path } = baseState;

        agast = yield sourceInstr;

        s = s.push(context, source.branch(), agast, balanced, spans);

        resolvedLanguages.set(s.path, resolvedLanguages.get(path));
