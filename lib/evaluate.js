...
import { printSource, streamFromTree, treeFromStream } from '@bablr/agast-helpers/tree';
import * as Tags from '@bablr/agast-helpers/tags';

import { getEmbeddedTag, getEmbeddedRegexMatcher } from '@bablr/agast-vm-helpers/deembed';
...
let mergePatterns = (a, b) => {
  ...
  let a_ = isString(a) ? [[a]] : parseRegexPattern(a.value);
  let b_ = isString(b) ? [[b]] : parseRegexPattern(b.value);

  return buildEmbeddedRegexMatcher(printSource(buildPattern([...a_, ...b_])));
};

let unwrapPattern = (pattern) => {
  return isString(pattern) ? pattern : getEmbeddedRegexMatcher(pattern);
};
...
function* __bablr(ctx, rootSource, rootLanguage, strategy, options) {
  ...
  for (;;) {
    ...
    switch (verb) {
      case 'write': {
        let { arguments: { 0: text, 1: { value: writeOptions } = { value: {} } } = [] } = instr;
        let { stream = 1 } = writeOptions;

        if (options.emitEffects) {
          if (!Number.isFinite(stream) || stream < 1 || stream > 4) throw new Error();
          yield `<-${stream}>`;
          yield* streamFromTree(text);
          yield '</>';
        }
        break;
      }
