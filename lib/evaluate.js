...
import { StreamGenerator, getCooked } from '@bablr/agast-helpers/stream';
...
function updateSpans(ctx, s, terminal) {
  const { attributes, flags } = terminal.value;
  const { innerSpan, balanced, balancedSpan, balancer } = attributes || {};
        ...
        switch (terminal?.type || 'Null') {
          ...
          case 'OpenNodeTag': {
            ...
            const resolvedLanguage =
              oldNode.depth > 1 ? resolvedLanguages.get(oldNode.parent) : rootLanguage;
          ...
          case 'CloseNodeTag': {
            ...
            const { balancedSpan, innerSpan, balanced } = attributes || {};
