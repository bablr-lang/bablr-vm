  ...
  for (;;) {
    ...
    switch (verb) {
        ...
        switch (tag.type) {
          ...
          case CloseNodeTag: {
            ...
            if (m.rangeInitial?.tag.type === ShiftTag) {
              let { shift } = m.rangeInitial.propertyWrapper.tag.value.property;
            ...
            if (node.tags.at(0).value.type) {
              ...
              s.agast = m.agastFragment || m.parent?.agast;

              // s.held = buildFacadeProperty(refPath?.tag.value, null, gaplessNode || m.node);

              m.agastFragment?.vm.next(buildNodeTag(gaplessNode?.node || m.node.node));
      ...
      case 'startFrame': {
        ...
        let offset = [ReferenceTag, ShiftTag, BindingTag].includes(lastWrapped?.type) ? -1 : 0;
      ...
      case 'endFrame': {
        ...
        if (finishedMatch.effects.success !== 'none') {
          s.agast = finishedMatch.agastFragment || m?.agast;
        } else {
