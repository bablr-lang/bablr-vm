...
import { offsetForTag, referencesAreEqual } from '@bablr/agast-helpers/path';
...
function* __bablr(ctx, rootSource, strategy, options) {
  ...
  for (;;) {
    ...
    if (co.done) {
      ...
      return co.value && states.get(co.value).node.node;
    }
    ...
    switch (verb) {
      ...
      case 'advance': {
        ...
        switch (tag.type) {
          ...
          case BindingTag: {
            ...
            // FIXME copypasta
            if (getOpenTag(propertyChild.value.node).value.selfClosing) {
              m.agast.vm.next(tag);
              m.agast.vm.next(propertyChild);

              s.resultPath =
                (m.agast.state.path || m.agast.state.resultPath?.path) &&
                TagPath.from(
                  m.agast.state.path || m.agast.state.resultPath?.path,
                  -1,
                  tag.type === sym.PropertyWrapper ? -1 : offsetForTag(tag),
                );
            } else {
              m.agastFragment.vm.next(tag);
              m.agastFragment.vm.next(propertyChild);
              s.resultPath =
                (m.agastFragment.state.path || m.agastFragment.state.resultPath?.path) &&
                TagPath.from(
                  m.agastFragment.state.path || m.agastFragment.state.resultPath?.path,
                  -1,
                  tag.type === sym.PropertyWrapper ? -1 : offsetForTag(tag),
          ...
          case OpenNodeTag: {
            ...
            let isFragment = !m.path;
            const refPath = isFragment ? null : s.agast.state.resultPath;
      ...
      case 'startFrame': {
        ...
        let lastWrapped = m.fragmentNode?.tags.at(-1, -1);
