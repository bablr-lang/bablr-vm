...
import { reifyExpression, reifyAttributes, buildNull } from '@bablr/agast-vm-helpers';
...
export function evaluate(ctxFacade, rootSourceFacade, strategy) {
  ...
  return __evaluate(ctx, rootSource, strategy);
}
  ...
  while (!co.done) {
    ...
    switch (verb) {
      case 'advance': {
        ...
        switch (matcher.type) {
          case 'String': {
            const text = reifyExpression(matcher);

            if (startsWithSeq(text, source)) {
              s.source.advance(text.length);

              terminal = buildLiteral(text);

              yield* s.emit(terminal);
            } else {
              throw new Error('Cannot advance, no match. This is a grammar problem');
            }
      ...
      case 'startNode': {
        ...
        const flags_ = (flags && reifyExpression(flags)) || {};
        const openTag = isFragment
          ? buildFragmentOpenTag(flags_)
          : buildNodeOpenTag(flags_, type_, reifyAttributes(attributes));
      ...
      case 'endNode': {
        ...
        if (type_ !== startType) throw new Error();
