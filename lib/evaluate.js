...
import { referencesAreEqual, TagPath } from '@bablr/agast-helpers/path';
...
const defineAttribute = (m, path, value) => {
  let nodeState = nodeStates.get(m.node);
    ...
    switch (verb) {
      ...
      case 'advance': {
        ...
        switch (tag.type) {
          ...
          case ReferenceTag: {
            if (s.held && sumtree.getAt(2, s.node.children)?.type === InitializerTag) {
              if (
                !referencesAreEqual(sumtree.getAt(1, s.node.children), tag) ||
                sumtree.getSize(s.node.children) > 3
              ) {
                throw new Error();
              }
            }
          ...
          case OpenNodeTag: {
            ...
            let nodeState = {
              undefinedAttributes: 0,
            };

            let parentAgast = s.agast;
            ...
            if (tag.value.type) {
              parentAgast.vm.next(buildEmbeddedNode(m.node));
            }

            let queue = [m.node.attributes];
            while (queue.length) {
            ...
            nodeStates.set(m.node, nodeState);
          ...
          case CloseNodeTag: {
            ...
            if (sumtree.getAt(0, node.children).value.type) {
              ...
              if (refPath?.tag.value.type === '@') {
                ...
                defineAttribute(m, ['cooked'], cooked);

                nodeStates.get(m.node).undefinedAttributes--;
              }
          ...
          case GapTag: {
            if (s.source.value == null && (!s.source.done || s.source.holding)) {
              ...
              if (s.held) {
                s.agast.vm.next(buildEmbeddedNode(s.held));
              ...
              if (s.expressions.size) {
                ...
                s.agast.vm.next(buildEmbeddedNode(getRoot(expression)));
      ...
      case 'defineAttribute': {
        const { arguments: { 0: path, 1: value } = [] } = instr;
