...
import { getProduction, unresolveLanguage } from '@bablr/helpers/grammar';
            ...
            if (s.source.value == null && (!s.source.done || s.source.holding)) {
              ...
              if (s.held) {
                ...
                s.agast.vm.next(
                  buildChild(
                    Property,
                    buildProperty(s.resultPath.tag.value, null, buildStubNode(tag)),
                  ),
                );
              ...
              if (s.expressions.size) {
                ...
                let bindingTag = buildBindingTag(unresolveLanguage(ctx, language, node.language));
                s.agast.vm.next(bindingTag);
                s.agast.vm.next(
                  buildChild(
                    Property,
                    buildProperty(s.resultPath.tag.value, bindingTag.value, node),
                  ),
                );
