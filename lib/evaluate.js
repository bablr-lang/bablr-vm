...
import { effectsFor, reifyBablrOptions } from '@bablr/agast-vm-helpers';
import { buildNode, get, list } from '@bablr/agast-helpers/path';
...
function* __bablr(ctx, rootSource, rootLanguage, strategy, options, registerNode) {
  ...
  for (;;) {
    ...
    switch (verb) {
      ...
      case 'advance': {
        ...
        switch (tag.type) {
          ...
          case ShiftTag: {
            ...
            s.holding = BTree.push(s.holding, lastProp.value.node);
          ...
          case OpenNodeTag: {
            ...
            if (selfClosing) {
              ...
              if (s.resultPath.depth >= 1) {
                ...
                s.holding = BTree.push(s.holding, s.node);
              } else {
                m.advance(tag, s);
              }
      ...
      case 'returnHeld': {
        let holdingMatch = BTree.getAt(-1, s.holdingMatches);

        holdingMatch.advance(s.held);
