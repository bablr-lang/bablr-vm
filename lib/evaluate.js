import { buildEscapeTag, buildGapTag, buildSpanEntry } from '@bablr/agast-helpers/builders';
import { parseLiteralTag, parseObject, parseTag, parseTagType } from '@bablr/agast-helpers/parsers';
import {
  isEmpty,
  printSource,
  streamFromString,
...
import {
  printObject,
  treeFromStream,
  printSource as printTreeSource,
} from '@bablr/agast-helpers/tree';
import { printTag, printType } from '@bablr/agast-helpers/print';
    ...
    switch (verb) {
      ...
      case 'advance': {
        ...
        switch (tag.type) {
          ...
          case OpenNodeTag: {
            ...
            if (selfClosing) {
              if (literalValue) {
                ...
                let result = yield* s.guardedMatch(buildEmbeddedStringMatcher(literalValue.value));
          ...
          case AttributeDefinition: {
            ...
            if (!m.options.escape && !has(m.node.value.attributes, path)) {
      ...
      case 'eatHeld': {
        ...
        if (s.shifted) {
          ...
          let tag_ = finishedMatch.shiftMatch ? Tags.getAt(-2, s.held.value.tags) : s.held;
          if (finishedMatch.options.escape) {
            tag_ = Tags.getAt(-2, s.held.value.tags);
            tag_ = printTag(
              buildEscapeTag(printTreeSource(s.held), tag_.value.node.value.attributes.cooked),
            );
          }
          m.advance(tag_);
        }
      ...
      case 'returnHeld': {
        ...
        // there is some condition under which we should not do this... ???
