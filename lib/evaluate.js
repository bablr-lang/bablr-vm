...
export const bablr = (ctx, rootSource, language, strategy, options = {}) => {
  return new StreamIterable(__bablr(ctx, rootSource, language, strategy, options));
};
...
function* __bablr(ctx, rootSource, rootLanguage, strategy, options) {
  ...
  for (;;) {
    ...
    switch (verb) {
      ...
      case 'init': {
        let { arguments: { 0: bindingMatchers } = [] } = instr;
        ...
        s = State.from(
          ...
          ctx,
          resolveLanguage(
            BTree.fromValues([rootLanguage]),
            bindingMatchers.flatMap((m) => m.segments),
          ),
          options.expressions,
        );
