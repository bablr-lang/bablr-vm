...
import { FragmentFacade, internalStates, states } from './node.js';
...
const defineAttribute = (m, path, value) => {
  internalStates.get(m.node).vm.next(buildAttributeDefinition(path, value));
      ...
      case 'startFrame': {
        ...
        nodeStates.set(m.node, {
          undefinedAttributes: m.isNode
            ? null
            : parentMatch
            ? nodeStates.get(parentMatch.node).undefinedAttributes ?? null
            : null,
        });
