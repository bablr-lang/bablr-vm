...
function __fromAst(matchNode, matchNodesByRef) {
  ...
  for (const token of cstTokens) {
    if (token.type === 'Reference') {
      ...
      const cstNode = __fromAst(matchNodesByRef.get(token), matchNodesByRef);
...
function fromAst(node, grammar, options) {
  ...
  return __fromAst(...traverse(node, grammar, options));
}
