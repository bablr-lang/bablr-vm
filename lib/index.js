...
const { match: _match, buildContext } = require('./match.js');
...
function updateTokens(node, options = {}) {
  ...
  __updateTokens(_match(node, source, context), matchNodes);
}
...
function print(node) {
  ...
  return __print(_match(node, source, context), matchNodes);
}
