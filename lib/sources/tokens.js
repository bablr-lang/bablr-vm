...
class TokensSourceFacade {
  ...
  *[_chrs]() {
  ...
  exec(pattern) {
    const { writable: wasWritable, source } = this[_];
    ...
    if (result) {
      ...
      while (this.chrIndex > chrIndex + result.length) {
        this[_actual].advanceChr(this.node, true);
      }
    }

    if (!wasWritable && this[_].writable) {
      if (result) {
      }
    }
...
class TokensSource {
  ...
  branch(lookahead = false) {
    // prettier-ignore
    const { isHoistable, stack, sourceNode, sourceTokens, descriptor, resolver, index, chrIndex, offset } = this;
  ...
  advance(node, backwards = false) {
    ...
    this.offset = mergeable && hasNext ? (backwards ? this.token.value.length - 1 : 0) : null;
  }
