...
export const printTerminal = (terminal) => {
  if (terminal.type === 'Literal') {
    ...
    const { type, attributes } = terminal.value;
  ...
  } else if (terminal.type === 'OpenFragmentNode') {
    return `<>`;
...
export const printCSTML = (node) => {
  ...
  for (const terminal of node.children) {
    if (terminal.type === 'Reference') {
      printed += `${terminal.value.pathName}:`;
...
export const streamPrintCSTML = (terminals, indent = '  ') => {
  if (!terminals) {
    return '<//>';
  }
...
export const streamPrintPrettyCSTML = (terminals, indent = '  ') => {
  ...
  for (const terminal of terminals) {
    ...
    if (['OpenFragmentNode', 'OpenNode'].includes(terminal.type)) {
      indentLevel++;
    }
