 ...
 */
export function* dispatcher(ctx, source, trampoline) {
  let m = null;
      ...
      switch (instr.type) {
        ...
        case sym.dispatch: {
          ...
          if (m) {
            ...
            co = new Coroutine(grammar.get(type).value(buildProps(ctx, instr.value, m)));
          } else {
            const gapTag = { type: 'GapTag', type, attrs };
            const path = Path.from(ctx, gapTag);
            const state = State.from(ctx);

            const match = Match.from(ctx, state, matchable);
