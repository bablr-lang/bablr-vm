...
function* __runGrammar(matchers, rootMatch, source) {
  let m = rootMatch;
  ...
  for (;;) {
    while (!m.co.done) {
      ...
      switch (instr.type) {
        case sym.dispatch: {
          const { state, ctx } = m;
          const { type, grammar, matchable, effects, stateUpdates } = instr.value;
          ...
          m = m.push(new Match(matchers, ctx, nextState, matchable));
...
function* __runLanguage(grammars, source) {
  const co = new Coroutine(__runGrammar(matchers, match, source));
}

export function* runSync(matchers, match, source) {
  let co = new Coroutine(__runLanguage(grammars, source));
