...
export class Match extends WeakStackFrame {
  ...
  static from(context, state, matchable) {
    ...
    const props = buildProps(context, matchable, state);

    const co = new Coroutine(getPrototypeOf(grammar)[tag.tagType].call(grammar, props));
  ...
  exec(instruction, state) {
    ...
    const tag = matchable.value;
    const { grammars } = ctx;
    const matchableGrammarType = getGrammarType(matchable);
    const grammar = grammars.get(matchableGrammarType);
    let scopedState = state;
    if (grammarType === sym.node && matchableGrammarType === sym.token) {
      scopedState = state.chrState;
      scopedState.status = sym.active;
    }

    const nextState = shouldBranch(effects) ? scopedState.branch() : scopedState;
    const props = buildProps(ctx, matchable, nextState, tag.attrs);
