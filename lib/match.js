...
import { buildPropertyTag, getCloseTag, has, Path, TagPath } from '@bablr/agast-helpers/path';
import { buildEscapeTag } from '@bablr/agast-helpers/builders';
import { parseTagType } from '@bablr/agast-helpers/parsers';
...
export class Match {
  ...
  get parentTagPath() {
    ...
    return nodePath.tagPathAt(tagsIndex + (this.rootNode.value.children[0] ? 1 : 0));
  }
  ...
  return() {
    ...
    if (finishedMatch.effects.success === 'eat') {
      let lastProperty = Tags.getAt(-2, finishedMatch.rootNode.value.tags);
        ...
        for (let tag of Tags.traverse(finishedMatch.rootNode.value.children)) {
          if (isObject(tag) && tag.type === Property) {
            ...
            if (
              ...
              m.advance(
                buildPropertyTag(
                  BList.fromValues([tags[1][0], '', '', buildNode(buildNullTag())], 1),
                ),
              );
            }
          }
      ...
      if (
        ...
        do {
          if (m.running) {
            ...
            tagPath = TagPath.fromNode(m.rootNode, shiftOffset)?.next;
