...
import { buildReference, mergeReferences } from '@bablr/agast-helpers/tree';
...
export class MatchFacade {
  ...
  get fragment() {
    ...
    const { name, isArray, flags } = mergedReference;
    ...
    if ((isNode || isCoverBoundary) && name) {
      return new FragmentFacade(
        ...
        mergedReference,
        isArray ? fragmentNode.getChildPropertyIndex(rangePreviousIndex + 1) : null,
      );
    } else {
  ...
  get rangeCurrentIndex() {
    return actuals.get(this).rangeCurrentIndex;
  }
...
export class Match extends WeakStackFrame {
  ...
  get rangePrevious() {
    let path = this.fragmentPath?.atDepth(0) || this.path;
  ...
  setRangeFinalIndex(value) {
    ...
    if (this.rangeFinal.tag.type === ReferenceTag) throw new Error();
  }

  get rangeCurrentIndex() {
    let lastTag = this.fragmentNode?.tags.at(-1);
    let lastWrapped =
      lastTag?.type === PropertyWrapper ? lastTag.value.tags[lastTag.value.tags.length - 1] : null;

    let offset = [ReferenceTag, ShiftTag].includes(lastWrapped?.type) ? -1 : 0;

    let previousIndex = [CloseNodeTag, NullTag, GapTag].includes(this.s.resultPath.tag.type)
      ? this.fragmentNode.tags.size - 1
      : this.s.resultPath.tagsIndex;
    return previousIndex + offset;
  }
  ...
  get referencePath() {
    ...
    if (ref.tag.type === ShiftTag) {
      let { shift } = ref.propertyWrapper.tag.value.property;
      let refIndex = ref.tagsIndex - shift.index;
  ...
  endFrame() {
    ...
    let lastTag = m.fragmentNode?.tags.at(-1);
    let lastWrapped =
      lastTag?.type === PropertyWrapper ? lastTag.value.tags[lastTag.value.tags.length - 1] : null;

    let offset = [ReferenceTag, ShiftTag].includes(lastWrapped?.type) ? -1 : 0;

    finishedMatch.setRangeFinalIndex((finishedMatch.fragmentNode || m.node).tags.size - 1 + offset);
    ...
    if (!state.depth) {
      ...
      while (tagPath) {
        ...
        if (
          ...
          tagPath = m.parent
            ? TagPath.from(
                m.fragmentPath || m.agastFragment.state.resultPath.path,
                m.rangeInitial.tagsIndex + 1,
                0,
              )
            : null;
