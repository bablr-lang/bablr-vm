...
import { getCloseNodeTag, has, isMultiFragment, Path, TagPath } from '@bablr/agast-helpers/path';
        ...
        for (let tag of Tags.traverse(finishedMatch.rootNode.value.children)) {
          if (isObject(tag) && tag.type === Property) {
            ...
            if (
              ...
              m.advance(
                buildPropertyTag(
                  BList.fromValues([tags[1][0], BList.empty(), buildNode(buildNullTag())], 1),
                ),
              );
            }
          }
