...
export class Match {
  ...
  constructor(
    ...
    if (!options.hasOwnProperty('shift')) throw new Error();
  ...
  push(context, state, rawMatcher, effects, shiftMatch, literalValue, options) {
    return new Match(this, context, state, rawMatcher, effects, shiftMatch, literalValue, options);
  }
  ...
  startFrame(state, rawMatcher, effects, shiftMatch, literalValue, options) {
    ...
    let m = this.push(context, state, rawMatcher, effects, shiftMatch, literalValue, options);
  ...
  endFrame() {
    ...
    if (finishedMatch.shiftMatch && s.shifted) {
      ...
      s.holdingMatches = BTree.pop(s.holdingMatches);
    }
    ...
    if (finishedMatch.effects.success === 'eat') {
      ...
      let returned;
      while ((returned = BTree.getAt(-1, s.returning))?.matchDepth === m.depth) {
        m.advance(returned.node);
        s.returning = BTree.pop(s.returning);
      }
      if (finishedMatch.isNode) {
        s.holding = BTree.push(
          s.holding,
          freeze({ matchDepth: m.depth, node: finishedMatch.rootNode }),
        );
        s.holdingMatches = BTree.push(s.holdingMatches, m);
      } else {
  ...
  throw_() {
    ...
    if (bind) {
      if (finishedMatch.type === '__') {
        for (let tag of Tags.traverse(finishedMatch.rootNode.value.children)) {
          if (tag.type === Property) {
            ...
            if (
              ...
              m.advance(buildPropertyTag([tags[0], [], buildNode(buildNullTag())]));
            }
          }
        }
      } else {
        m.advance(
          buildPropertyTag([
            buildChild(ReferenceTag, finishedMatch.mergedReference),
            [],
  ...
  *emit(options) {
    ...
    let { state, emitted } = emittedMatch;
