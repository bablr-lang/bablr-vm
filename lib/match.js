...
export class Match extends WeakStackFrame {
  ...
  constructor(parent, context, state, rawMatcher, effects, shiftMatch = null, options = {}) {
    ...
    this.languageMatch =
      matcher.bindingMatcher?.languagePath.length || !parent ? this : parent.languageMatch;
    this.agast = null;
    this.cover =
    ...
    this.language = parent
      ? resolveLanguage(state, matcher.bindingMatcher?.languagePath ?? [])
      : state.language;

    if (!this.language) {
      throw new Error(
        `Unknown language ${printIdentifierPath(matcher.bindingMatcher.languagePath)}`,
      );
    }
  ...
  startFrame(state, rawMatcher, effects, shiftMatch, options) {
    ...
    let { languagePath } = m.propertyMatcher.bindingMatcher || { languagePath: [] };

    for (let segment of languagePath) {
      language = segment.type
        ? BTree.getAt(-2, m.state.languages)
        : language.dependencies[segment.name];
      m.state.languages = BTree.push(m.state.languages, language);
    }
  ...
  endFrame() {
    ...
    let { languagePath } = finishedMatch.propertyMatcher.bindingMatcher || { languagePath: [] };
