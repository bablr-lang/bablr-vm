...
export class Match {
  ...
  get allowEmpty() {
    return !!BSet.has(this.name, this.grammar.emptyables) || this.options.allowEmpty;
  }
    ...
    for (let bindingMatcher of bindingMatchers) {
      ...
      for (let segment of segments) {
        language = segment.type
          ? BList.getAt(-2, state.languages)
          : language.dependencies[segment.name];
      }

      state.languages = BList.push(state.languages, language);
    }
        ...
        for (let tag of Tags.traverse(finishedMatch.rootNode.value.children)) {
          if (tag.type === Property) {
            ...
            if (
              ...
              m.advance(
                buildPropertyTag(
                  Tags.fromValues([tags[1][0], Tags.fromValues([]), buildNode(buildNullTag())]),
                ),
              );
            }
          }
