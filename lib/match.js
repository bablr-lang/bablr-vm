...
export class Match extends WeakStackFrame {
  static startFrame(ctx, s, m, finishedMatch, verb, matcher, options) {
    ...
    if (m.name !== sym.fragment && !getProduction(m.grammar, m.name))
  ...
  constructor(parent, context, state, rawMatcher, effects, shiftMatch = null, options = {}) {
    ...
    this.agast.vm.next(
      buildOpenNodeTag(parentHasGap ? getFlagsWithGap(fragmentFlags) : fragmentFlags),
    );
    if (!this.parent) {
      if (isNode || isCover) {
  ...
  endFrame() {
    ...
    if (m.emitted?.match === m && !finishedMatch.s.depth) {
      m.emitted = {
        match: m,
        tagPath: endsNode(m.emitted.tagPath.tag)
          ? m.emitted.tagPath.path.parentPropertyPath.nextSibling.propertyPath
          : m.emitted.tagPath.nextSibling
          ? // sometimes this is wrong
            m.emitted.tagPath.nextSibling.propertyPath
          : m.emitted.tagPath,
      };
    }
  ...
  throw_() {
    ...
    // ) {
    //   m.advance(buildNodeTag(finishedMatch.shiftMatch.rootNode.node));
  ...
  *emit(options) {
    ...
    if (!state.depth) {
      ...
      let tagPath =
        emitted?.tagPath || (Tags.getSize(getTags(node)) ? TagPath.fromNode(node, 0) : null);
      ...
      while (tagPath) {
        ...
        let holdShifted =
          options.holdShiftedNodes && tagPath.propertyPath?.tag.value.reference.flags.expression;
        // tagPath.path.referenceTagPath?.tag.value.flags.expression;

        let doneShifting =
          !holdShifted ||
          (tagPath.path.tagPathAt(-1, 0).tag.type !== ShiftTag &&
            tagPath.path.tagPathAt(-1, 0).tag !== tagPath.propertyPath?.tag.value.tags[0]);

        if (!doneShifting) break;

        if (!m.emitted || (!tagPath.equalTo(m.emitted.tagPath) && tagPath.tag.type !== Property)) {
        ...
        if (
          ...
          (!holdShifted && !tagPath.nextSibling && !tagPath.tag.type === Property)
        ) {
