...
export class Match extends WeakStackFrame {
  ...
  setRangePreviousIndex(value) {
    ...
    if (
      value != null &&
      this.isNode &&
      ![OpenNodeTag, Property, InitializerTag, AttributeDefinition].includes(
  ...
  get referencePath() {
    if (!this.isNode || this.state.depths.path < 0) {
      return null;
    }

    let ref = TagPath.fromNode(this.fragmentNode || this.node, this.rangePreviousIndex + 1);
  ...
  startFrame(state, propertyMatcher, effects, shiftMatch, options) {
    ...
    if (bindingMatcher) {
      language = resolveLanguage(context, this.language, bindingMatcher.languagePath);
