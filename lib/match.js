...
export class Match extends WeakStackFrame {
  ...
  get grammar() {
    ...
    return grammars.get(getGrammarType(this.matchable));
  }
  ...
  exec(instr) {
    const { ctx, grammar, state } = this;
    const { effects, matchable } = instr;
    const tag = matchable.value;

    const co = new Coroutine(grammar.get(tag.type).value(buildProps(ctx, matchable, state)));
