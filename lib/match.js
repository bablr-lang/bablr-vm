...
export class Match extends WeakStackFrame {
  static startFrame(language, ctx, s, m, finishedMatch, verb, matcher, options) {
    ...
    m = parentMatch
      ...
      : Match.from(ctx, language, s, matcher, null, options);
  ...
  startFrame(state, rawMatcher, effects, shiftMatch, options) {
    let { context } = this;
    const { bindingMatcher } = reifyExpression(rawMatcher);
    ...
    let m = this.push(context, language, state, rawMatcher, effects, shiftMatch, options);
