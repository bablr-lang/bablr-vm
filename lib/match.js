...
import { buildPropertyTag, getCloseNodeTag, has, Path, TagPath } from '@bablr/agast-helpers/path';
...
export class Match {
  ...
  get parentTagPath() {
    ...
    return Path.from(this.parent.rootNode)
      .childPathAt(-1)
      .inner.tagPathAt(tagsIndex + 1);
  }
  ...
  *emit(options) {
    ...
    while (tagPath) {
      ...
      if (!m.emitted || !tagPath.equalTo(this.emitted)) {
        if (tagPath.type !== Property && tagPath.type !== EmptyTag) {
          this.emitted = emitted = tagPath;
          this.emittedMatch = m;
