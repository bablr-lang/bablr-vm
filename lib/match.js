...
export class Match extends WeakStackFrame {
  constructor(parent, context, language, state, matcher, effects, shiftMatch = null, options = {}) {
    ...
    if (isCoverBoundary) {
      this.agast = agast({ held: state.held?.node });
    } else {
  ...
  advance(tag, s = this.state) {
    ...
    if (tag.type === GapTag && s.expressions.size) {
      result = vm.next(buildEmbeddedNode(s.expressions.value));
    ...
    if (!state.depth) {
      ...
      while (tagPath) {
        ...
        if (tagPath.tag.type === CloseNodeTag && !tagPath.next) {
          tagPath = TagPath.fromNode(m.fragmentNode, m.rangeInitial.childrenIndex + 2);
