...
export class Match extends WeakStackFrame {
  ...
  get suppressNode() {
    return this.options.suppressNode;
  }
  ...
  get unshiftedReferencePath() {
    ...
    if (this.didShift) {
      let refIndex = previous.childrenIndex - 1 - previous.tag.value.index * 2;
      return previous.siblingAt(refIndex);
    }
    ...
    if (finishedState !== m.state) {
      if (
        ...
        finishedState.reject(
          finishedMatch.fragmentNode,
          !finishedMatch.isNode || finishedMatch.effects.success === 'none',
        );
