...
import { facades, actuals, buildFacadeNode } from './facades.js';
import {
  CloseNodeTag,
  OpenNodeTag,
  AttributeDefinition,
...
export class MatchFacade {
  ...
  getPublic() {
    return actuals.get(this).getPublic();
  }
}
...
export class Match extends WeakStackFrame {
  ...
  *emit(options) {
  ...
  getPublic() {
    ...
    let getParent = () => {
      let value = parent;
      if (parent) parents.set((value = parents.get(parent) || parent.getPublic()));
      return value;
    };

    let getCover = () => {
      let value = cover;
      if (cover) parents.set((value = parents.get(cover) || cover.getPublic()));
      return value;
    };
    ...
    let getCoveredBoundary = () => {
      ...
      if (coveredBoundary)
        parents.set((value = parents.get(coveredBoundary) || coveredBoundary.getPublic()));
      return value;
    };

    let getNodeMatch = () => {
      let value = nodeMatch;
      if (nodeMatch) parents.set((value = parents.get(nodeMatch) || nodeMatch.getPublic()));
      return value;
    };

    let getShiftMatch = () => {
      let value = shiftMatch;
      if (shiftMatch) parents.set((value = parents.get(shiftMatch) || shiftMatch.getPublic()));
      return value;
    };
