...
const __match = (node, source, context) => {
    ...
    if (type === 'emit') {
      ...
      for (const descriptor of descriptors) {
        ...
        if (descriptor.type === 'Reference') {
          ...
          // Recurse!
          const tree = __match(child, childSource, context);
          // Done recursing. Yay!
      ...
      if (type === 'match') {
        ...
        if (match_) {
          ...
          return __match(node, fallbackSource, context);
        }
      } else {
...
const match = (node, source, context = buildContext()) => {
  const result = __match(node, source, context);
