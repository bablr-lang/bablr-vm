...
export class Match extends WeakStackFrame {
  ...
  constructor(parent, context, state, rawMatcher, effects, shiftMatch = null, options = {}) {
    ...
    this.languageMatch =
      matcher.bindingMatcher?.segments.length || !parent ? this : parent.languageMatch;
    this.agast = null;
    this.cover =
  ...
  startFrame(state, rawMatcher, effects, shiftMatch, options) {
    ...
    let { segments } = matcher.bindingMatcher || { segments: [] };
  ...
  endFrame() {
    ...
    let { segments } = finishedMatch.propertyMatcher.bindingMatcher || { segments: [] };
