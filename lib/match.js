...
import { buildProperty, buildReferenceTag, mergeReferenceTags } from '@bablr/agast-helpers/tree';
...
export class Match extends WeakStackFrame {
  ...
  get unshiftedReferencePath() {
    let previous = this.rangePrevious;
    ...
    if (previous.tag.type === DoctypeTag) {
    ...
    if (this.parent?.cover) {
      ...
      let refIndex = previous.childrenIndex - previous.nextSibling.tag.value.index * 3 + 1;
      ref = previous.siblingAt(refIndex);
    } else {
      ref = previous.nextSibling;
    }
    if (ref && ![ShiftTag, ReferenceTag].includes(ref.tag.type)) throw new Error();
    return ref;
  }
  ...
  advance(tag, s = this.state) {
    ...
    let result;
    if (tag.type === GapTag && s.expressions.size) {
      result = vm.next(buildProperty(reference, binding, s.expressions.value));
  ...
  startFrame(state, propertyMatcher, effects, didShift, options) {
    ...
    let m = this.push(context, language, state, propertyMatcher, effects, didShift, options);
  ...
  *emit(options) {
    ...
    if (!state.depth) {
      ...
      while (tagPath) {
        ...
        if (
          tagPath.tag.type === OpenNodeTag &&
          m.unshiftedReferencePath?.tag.value.type === '@' &&
        ...
        if (!state.emitted || !tagPath.equalTo(state.emitted.tagPath)) {
          ...
          if (
            ...
            !(options.holdUndefinedAttributes || m.node.children.at(-1)?.type === CloseNodeTag)
          ) {
            yield tagPath.tag;
          }
        }
