...
export class Match extends WeakStackFrame {
  ...
  startFrame(state, rawMatcher, effects, shiftMatch, options) {
    ...
    if (m.propertyMatcher.bindingMatcher?.languagePath.length) {
      m.state.languages = BTree.push(m.state.languages, m.language);
    }
  ...
  endFrame() {
    ...
    if (finishedMatch.propertyMatcher.bindingMatcher?.languagePath.length) {
