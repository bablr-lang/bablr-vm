...
export class MatchFacade {
  ...
  get fragment() {
    ...
    if (isNode && !['#', '@'].includes(name)) {
      return new FragmentFacade(
        ...
        mergedReference,
        isArray ? path.referenceIndexes[rangePreviousIndex + 1] : null,
      );
    } else {
      return new FragmentFacade(fragmentNode, ctx, false, [
  ...
  get shiftNode() {
    return facades.get(actuals.get(this).shiftNode);
  }
...
export class Match extends WeakStackFrame {
  ...
  get isCoverBoundary() {
    return (this.isNode && !this.cover) || this.cover === this;
  }
