...
import { freezeRecord, isObject, isSymbol } from '@bablr/agast-helpers/object';
...
export class Match {
  ...
  return() {
    ...
    if (finishedMatch.effects.success === 'eat') {
      let shiftProperty = Tags.getAt(-2, finishedMatch.rootNode.value.tags);

      s.node = finishedMatch.shiftMatch ? shiftProperty.value.node : finishedMatch.rootNode;
      ...
      if (finishedMatch.isNode) {
        ...
        m.advance(finishedMatch.shiftMatch ? shiftProperty : finishedMatch.rootNode);
        ...
        for (let tag of Tags.traverse(finishedMatch.rootNode.value.children)) {
          if (isObject(tag) && tag.type === Property) {
            ...
            if (
              ...
              m.advance(
                buildPropertyTag(
                  BList.fromValues([tags[1][0], Tags.fromValues([]), buildNode(buildNullTag())], 1),
                ),
              );
            }
          }
    ...
    while (tagPath) {
      ...
      if (holdingShifted) {
        ...
        if (
          ...
          let nextTagPath =
            previousTagPath && TagPath.fromNode(match.node, previousTagPath.tagsIndex + 2);
          ...
          while (!doneShifting && nextTagPath) {
            let next =
              TagPath.fromNode(match.node, [previousTagPath.tagsIndex + 2 + shiftIndex, 1]) ||
              TagPath.fromNode(match.node, previousTagPath.tagsIndex + 2 + shiftIndex);
      ...
      if (!m.emitted || !tagPath.equalTo(this.emitted)) {
        if (
          !(
            ...
            (m.depth || m.matcher.type !== Symbol.for('__'))
          )
        ) {
          this.emitted = emitted = tagPath;
          this.emittedMatch = m;
