  ...
  *emit(options) {
    ...
    if (!state.depth) {
      ...
      while (tagPath) {
        ...
        let holdingShifted = options.holdShiftedNodes && !!m.expressionMatch;
        // tagPath.path.referenceTagPath?.tag.value.flags.expression;

        if (holdingShifted) {
          if (m.expressionMatch.running || !getCloseTag(m.expressionMatch.rootNode)) {
            if (m.expressionMatch === m && tagPath.tag.type !== ReferenceTag && tagPath.next) {
        ...
        if (
          ...
          do {
            if (m.running) {
              m = m.running;
              tagPath = m.emitted.tagPath || TagPath.fromNode(m.rootNode, 1, 0);
