...
export const match = (pattern, source) => {
  if (typeof pattern === 'string' && pattern.length === 1) {
    return pattern === source.value
      ...
      : null;
  } else if (typeof pattern === 'string' || pattern.value.name === null) {
    let pattern_ =
      typeof pattern === 'string'
        ? pattern
        : sourceFromTokenStream(Tags.traverse(Tags.getTags(pattern)));
  ...
  const pattern_ =
    pattern.value.type === null && pattern.value.flags.token
  ...
  const iter = getStreamIterator(
    generateMatches(buildEmbeddedMatcher(pattern_), wrapSource(source)),
  );
