...
export const buildReferenceTag = (pathName) => {
  return freezeSeal({ type: 'Reference', value: freezeSeal({ pathName }) });
};

export const buildNodeOpenTag = (type, attributes = {}) => {
  return freezeSeal({ type: 'OpenNode', value: freezeSeal({ type, attributes }) });
};

export const buildNodeCloseTag = (type) => {
  return freezeSeal({ type: 'CloseNode', value: freezeSeal({ type }) });
};
