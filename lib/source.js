...
class ForkIterator {
  static from(fork) {
    return fork.done ? facades.get(fork) ?? new ForkIterator(fork) : new ForkIterator(fork.clone());
  }

  constructor(fork) {
    facades.set(fork, this);
  }
  ...
  next() {
    ...
    if (!fork.done) {
      const { head } = fork;
      fork.advance();
      return maybeWait(fork.head.step, () => {
        return head.step;
      });
    } else {
      return { value: undefined, done: true };
...
class Fork {
  ...
  return() {
    ...
    if (exchange) exchange.releaseFork(this);
...
export const Source = class BABLRSource extends WeakStackFrame {
  ...
  branch() {
    const { done, fork, exchange, index, holding } = this;
    return this.push(done ? fork : exchange.allocateFork(fork), exchange, index, holding);
  }
  ...
  [Symbol.for('@@streamIterator')]() {
    return (this.holding ? emptyStreamIterator() : this.fork)[Symbol.for('@@streamIterator')]();
  }
