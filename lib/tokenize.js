...
export function tokenize(language, sourceText, type, attrs = {}) {
  const context = Context.from(language);
  const source = Source.from(context, sourceText);
  const state = DispatcherState.from(
    context,
    new Map([[sym.token, TokenizerState.from(context, source)]]),
  );
  const trampolines = new Map([[sym.token, execToken]]);
  const matchable = { type: sym.token, production: { type, attrs } };
