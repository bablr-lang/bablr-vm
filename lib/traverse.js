        ...
        case 'reject': {
          ...
          if (nextState) {
            ...
            try {
              ...
              state = new State(path, state.Stateource.fallback());
              co = path.co = new Coroutine(state);
            } catch (e) {
              const cause = failedCommand.error;
              let from = '';
              if (failedCommand.type === 'take') {
                from += ` from {source: ${state.source.facade.toString()}}`;
              }
              throw new Error(
                `cst-tokens failed executing {command: ${formatCommand(failedCommand)}}${from}`,
                cause && { cause },
              );
            }
          }
