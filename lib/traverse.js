    ...
    while (!s.grammar.done) {
      ...
      switch (type) {
        ...
        case eatGrammar: {
          ...
          debug(indent(s, cmdName), grammar.name);
        ...
        case eat: {
          ...
          if (!s.source) {
            debug(indent(s, cmdName), descriptor.type, type !== eat ? '[+]' : '');
            result = descriptor.build();
          } else if (
            s.source.type === 'TokensSource' &&
            (s.source.done || s.source.token.type !== descriptor.type)
          ) {
            debug(indent(s, cmdName), descriptor.type);
            result = null;
          } else {
            [s, result] = matchDescriptor(command.value, s);

            debug(indent(s, cmdName), descriptor.type, type !== eat && result ? '[+]' : '');
