      ...
      switch (instr.type) {
        ...
        case eat: {
          ...
          switch (matchable.type) {
            case sym.node: {
              const { property, value } = matchable.value;
              let { type } = matchable.value;
              ...
              if (ctx.nodeGrammar.is('Node', type)) {
                ...
                if (!child) {
                  throw new Error(
                    `failed to resolve ${formatType(property)} on ${formatType(s.node.type)}`,
                  );
                }

                if (!ctx.nodeGrammar.is(type, child.type)) {
                  if (instr.type === eat) {
                    s.reject();
                  } else {
                    returnValue = null;
