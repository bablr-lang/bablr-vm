...
export function* traverse(language, node, context = Context.from(language)) {
  yield* traverseFragment(language, node, context);
...
export function traverseFragment(language, node, context = Context.from(language)) {
  ...
  return runSync(context, match, __traverse(match));
}
        ...
        case match: {
          ...
          switch (matchable.type) {
            ...
            case sym.token: {
              ...
              if (!shouldBranch && !range && successEffect === sym.fail) {
                m.terminate();
              }
