...
function __traverse(context, initialState) {
  ...
  for (;;) {
          ...
          switch (matchable.type) {
            ...
            case sym.token: {
              ...
              switch (type) {
                ...
                default: {
                  let { tokenizer: tokenizerState } = s;

                  const range = tokenize(ctx, tokenizerState, type, value, instr.type);
    ...
    // a production coroutine has terminated
