...
const { TextSourceFacade } = require('./sources/text.js');
const { TokensSourceFacade } = require('./sources/tokens.js');
const { State } = require('./state.js');
const { GrammarFacade } = require('./grammar.js');
...
const { buildCSTNode } = require('./cst.js');

const traverseFragment = (node, grammar, options = {}) => {
  const { sourceText: text } = options;
  const source = text != null ? TextSourceFacade.from(text) : TokensSourceFacade.from(node);
  const grammar_ = GrammarFacade.from(grammar);
  const fragment = fragmentNodeFor(node, source);
  const path = PathFacade.from(fragment);
  const production = Production.fromPath(path, grammar_);
  const rootState = new State(path, source, production);

  if (debugTree.enabled) debugTree('  -> CSTFragment');

  const result = traverse(grammar_, rootState);
  ...
  return buildCSTNode(null, fragment, [...result], grammar_);
};
    ...
    while (!s.production.done) {
      ...
      switch (type) {
        ...
        case eatProduction: {
          ...
          s.production = new Production(production, grammar);
          s.result = emptyStack;
          s.depth++;
        ...
        case eat: {
          ...
          if (result) {
            ...
            token = { type: descriptor.type, value: result };
