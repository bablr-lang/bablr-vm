...
import { freezeSeal, get, isString, isType } from './utils/object.js';
    ...
    while (s.status === active && !m.co.done) {
      ...
      switch (instr.type) {
        ...
        case startNode: {
          ...
          ctx.prevTokens.set(startNodeToken, s.lastToken);
          ctx.pathsMap.set(startNodeToken, path);

          s.path = path;
          s.resolvers.set(s.path, Resolver.from(s.node));
          s.tokenState.lastToken = startNodeToken;
