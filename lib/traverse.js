...
function __traverse(ctx, initial) {
  ...
  for (;;) {
          ...
          switch (matchable.type) {
            case sym.node: {
              ...
              if (ctx.nodeGrammar.is('Node', type)) {
                ...
                if (!child) throw new Error(`failed to resolve ${formatType(property)}`);
              ...
              s.pushCo(
                Coroutine.from(ctx.nodeGrammar, type, {
                  context: facades.get(ctx),
                  state: facades.get(s),
                  value,
                  property,
    ...
    {
      const range = ctx.getRangeFromPreviousAndFinal(ctx.prevTokens.get(s.co), s.result);
