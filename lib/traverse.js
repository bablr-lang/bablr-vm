              ...
              switch (type) {
                ...
                case EOF: {
                  if (s.tokenizer.done) {
                    ...
                    // if eat, store this result and possibly fail!
                    // move into tokenizer?
                ...
                default: {
                  const tokenizer = instr.type === eat ? s.tokenizer : s.tokenizer.branch();

                  const range = tokenize(ctx, tokenizer, type, value);

                  if (range && instr.type !== eat) {
                    s.tokenizer.accept(tokenizer);
                  }
