...
const traverse = (node, grammar, options) => {
  ...
  for (;;) {
    ...
    while (!co.done) {
      ...
      switch (type) {
        ...
        case 'reject': {
          ...
          if (!state) {
            // We failed!
            try {
              path = new Path(path.node, context, path.parent);
        ...
        case 'traverse': {
          if (command.value !== refToken) {
            throw new Error('Invalid ref command');
          }

          const name = command.value.value;
          ...
          path = new Path(child, context, path, state);
          state = new State(path, state.source.branch(child), new Resolver(child));
          co = path.co = new Coroutine(state);

          matchNodes.set(refToken, path.matchNode);
          refToken = undefined;
        ...
        case 'match': {
          ...
          if (descriptor.type === 'Reference') {
            ...
            refToken = descriptor.build();
            returnValue = [refToken];
            refTokenUpdated = true;
      ...
      // Ensure that match(ref`name`) must directly precede ref(name)
      if (!refTokenUpdated) {
        if (refToken) {
          throw new Error('You must traverse into a reference');
        }
      }
