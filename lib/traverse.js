...
const { arraySlice } = require('./utils/array.js');
...
const { State, getParentNodeState } = require('./state.js');
...
const traverse = (context, rootState) => {
  ...
  if (debug.enabled && rootState.path !== rootState.parent?.path) {
    debug(' ->', formatNodeType(s.path.node));
  }
  ...
  for (;;) {
    while (!s.grammar.done) {
      ...
      switch (type) {
        ...
        case eatGrammar: {
          ...
          const parentNodeState = getParentNodeState(s, rootState);
          const initialResult = s.result;
          const { result } = s.hoisting && parentNodeState ? parentNodeState : s;
          ...
          const matchResult = traverse(context, s);

          if (s.status === 'rejected') {
            s = s.parent;
          } else if (matchResult && type !== matchGrammar) {
            s = s.accept();
            s.result = initialResult;
            if (s.hoisting && parentNodeState) {
              parentNodeState.result = result.concat(matchResult);
            } else {
        ...
        case eat: {
          ...
          if (result) {
            ...
            if (type !== match) {
              const parentNodeState = getParentNodeState(s, rootState);
              if (s.hoisting && parentNodeState) {
                parentNodeState.result = parentNodeState.result.push(token);
              } else {
        ...
        case reference: {
          ...
          if (debug.enabled) debug(' ->', formatNodeType(child));
        ...
        case startNode: {
          ...
          if (debug.enabled) debug(indent(s, cmdName));
    ...
    if (rootState.status !== 'rejected' && (!s.parent || s.path !== s.parent.path)) {
      ...
      return s.result.size ? [...s.result] : null;
    }
  }
};
