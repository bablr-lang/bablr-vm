...
const traverse = (node, grammar, options = {}) => {
  ...
  if (leading.length || trailing.length) {
    throw new Error('Fragment must not hoist tokens');
  }

  const fragmentRefIdx = own.findIndex((t) => t.type === 'Reference');

  if (fragmentRefIdx < 0) {
    throw new Error('fragment grammar must eat ref`fragment`');
  }
  const innerMatchNode = matchNodesByRef.get(own[fragmentRefIdx]);
  const innerTokens = innerMatchNode.cstTokens;

  own.splice(fragmentRefIdx, 1, ...innerTokens);
  innerMatchNode.cstTokens = own;
