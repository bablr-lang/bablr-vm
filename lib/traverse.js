...
const traverseFragment = (node, grammar, options = {}) => {
  ...
  const state = new State(path, context[_actual].source);
  state.co = new Coroutine(generatorFor(state, context));
...
const traverse = (context, initialState) => {
  ...
  for (;;) {
    while (!s.co.done) {
      ...
      switch (type) {
        ...
        case 'takeGrammar': {
          ...
          s = s.branch();
          s.co = new Coroutine(Grammar(s.path, context, s));
          s.result = emptyStack;
        ...
        case 'take': {
          ...
          if (descriptor.type === 'Reference') {
            ...
            s.co = new Coroutine(generatorFor(s, context));
            s.result = emptyStack;
    ...
    s.result = s.co.value !== undefined ? asStack(s.co.value) : s.result;
