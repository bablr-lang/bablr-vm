...
export function* traverse(language, node, source, context = Context.from(language)) {
  ...
  yield* __traverse(context, { node, state });
...
function* __traverse(ctx, initial) {
  ...
  {
    const initialPath = Path.from(grammar, Fragment, initial.node);

    s.pushCo(
      Coroutine.from(grammar, initial.node.type, {
  ...
  for (;;) {
    ...
    while (s.status === active && !s.co.done) {
      ...
      const { error: cause, branch: shouldBranch } = instr;
      ...
      switch (instr.type) {
        ...
        case eat: {
          ...
          const shouldCommit = instr.type === eat;

          if (instr.type === match && !shouldBranch) throw new Error();
          ...
          switch (matchable.type) {
            case sym.node: {
              ...
              if (grammar.is('Node', type)) {
                ...
                path = Path.from(grammar, type, child, s.path, resolvedProperty);
              } else {
                if (shouldBranch) {
                  s = s.branch(shouldCommit);
                }
              }

              s.pushCo(
                Coroutine.from(grammar, path ? path.node.type : type, {
                  context: facades.get(ctx),
                  state: facades.get(s),
                  value,
                  property,
        ...
        case fail: {
          if (shouldBranch) throw new Error();
        ...
        case startNode: {
          ...
          const { value: type } = instr;

          const path = ctx.paths.get(s.co);

          if (type !== path.node.type) throw new Error();

          const startNodeToken = createToken(StartNode, path.node.type);
