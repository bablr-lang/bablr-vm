...
export function* parseTrampoline(ctx, rootState, rootMatcher, rootProps) {
...

  {
    ...
    const isTerminalNode = rootMatcher.type === 'TerminalMatcher';

    if (isNode && !isCover) {
      yield buildCall('startNode', type, rootMatcher.attributes, isTerminalNode);
    }
  }
      ...
      switch (verb) {
        ...
        case 'guard': {
          ...
          if (['NodeMatcher', 'TerminalMatcher'].includes(matcher.type)) {
            ...
            if ((isNode || isCover) && s.result.type !== 'ReferenceTag') {
