...
import { tokenTag, openNodeTag, closeNodeTag } from './utils/ast.js';
...
export function parse(language, sourceText, gapType, attrs = new Map()) {
  ...
  const matchable = { type: 'GapNodeTag', value: { type: gapType, attrs } };
          ...
          if (matchable.type === 'GapNodeTag') {
            ...
            if (nodeGrammar.is('Node', type)) {
              ...
              if (!nodeGrammar.aliases.has(type)) {
                const tag = openNodeTag(s.path.isGap ? s.path.gapType : type, type, attrs);
            ...
            returnValue = defer;
          } else if (matchable.type === 'GapTokenTag') {
    ...
    switch (m.grammarType) {
      case sym.node:
        if (!m.empty && m.grammar.is('Node', m.type)) {
          yield emit(closeNodeTag(m.matchable.value.type));
        }
