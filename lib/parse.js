...
export function parse(language, sourceText, matcher) {
  ...
  for (const terminal of terminals) {
    switch (terminal.type) {
      case 'OpenNode': {
        const { language, type, path, attributes } = terminal.value;
        ...
        // .[path]
        set(node.properties, path, newNode);
  ...
  while (m) {
      ...
      switch (verb) {
        ...
        case 'guard': {
          ...
          if (matcher.type === 'NodeMatcher' || matcher.type === 'TerminalMatcher') {
            ...
            m = m.exec(state, effects, matcher, strPath, buildExpression(props));
...

    {
      ...
      if (!m.parent) {
        const type = getCooked(rootMatcher.properties.type);
        if (m.grammar.covers.get(sym.node).has(type) && !m.grammar.covers.has(type)) {
          yield buildCall('emit', buildNodeCloseTag(sym.Document));
        }
      }
