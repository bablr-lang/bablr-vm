...
export const State = class BABLRState extends WeakStackFrame {
  ...
  guardedMatch(pattern) {
    ...
    if (pattern_?.intrinsicValue) {
      pattern_ = pattern_.intrinsicValue || getCooked(pattern_.tags);
  ...
  branch() {
    ...
    newAgast.vm.next(
      buildChild(
        ...
        buildPropertyWrapper(
          [fragRefTag],
          buildProperty(fragRefTag.value, null, node.tags.childrenNode),
        ),
      ),
    );
  ...
  accept() {
    ...
    if (parent.depths.path === accepted.depths.path) {
      ...
      for (let i = parentChildren.size; i < accepted.node.tags.size; i++) {
        let tag = accepted.node.tags.at(i);
        ...
        for (let i = parentChildren.size; i < ourChildren.size; i++) {
          ...
          if ([InitializerTag, GapTag].includes(tag.type)) {
            ...
            if (isShift) {
              ...
              referenceTag = ourChildren.at(refIndex);
            }
