...
import { buildGapTag, buildReferenceTag, buildStubNode } from '@bablr/agast-helpers/tree';
import {
  get,
  getShifted,
  Path,
...
export const StateFacade = class BABLRStateFacade {
  ...
  get held() {
    ...
    return held && FragmentFacade.wrap(held, this.ctx);
  }
    ...
    if (parent.depths.path === accepted.depths.path) {
      ...
      for (let i = btree.getSum(parentChildren); i < btree.getSum(accepted.node.children); i++) {
        ...
        if (tag.type === GapTag) {
          let referenceIdx = i;
          let reference;
          let previousSibling = btree.getAt(i - 1, accepted.node.children);
