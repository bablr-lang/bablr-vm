...
import { getRoot, nodeIsComplete } from '@bablr/agast-helpers/path';
...
export const State = class BABLRState {
  ...
  *guardedMatch(pattern) {
    ...
    if (isString(pattern)) {
    } else if (pattern.type === Matcher) {
      if (pattern.value.type !== TreeNode) throw new Error();
      let node = getRoot(pattern.value);
      let { name, flags, type } = node.value;

      if (name === Symbol.for('String')) {
        pattern_ = reifyExpression(node);
      } else if (name === Symbol.for('Pattern')) {
        pattern_ = node;
      } else if (name == null && type == null && flags.token) {
        pattern_ = node;
      } else {
        throw new Error();
      }
