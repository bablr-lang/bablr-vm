    ...
    switch (verb) {
      ...
      case 'reject':
        ...
        if (rejectedState.path.depth > s.path.depth) {
          ...
          if (pathIsArray && !s.path.resolver.counters.has(pathName)) {
      ...
      case 'startNode': {
        ...
        const openNodeTag = isFragment
          ? buildFragmentOpenTag()
          : buildNodeOpenTag(type, attributes);
        ...
        const { spanInside, balancer } = s.path;
        ...
        s.path.pushTag(openNodeTag);

        ctx.tagPaths.set(openNodeTag, s.path);

        yield* s.emit(openNodeTag);

        returnValue = openNodeTag;
        break;
      }
      ...
      case 'endNode': {
        ...
        const { startTag, reference } = s.path;
        const endTag = buildNodeCloseTag(type);

        if (startTag.type !== 'OpenFragmentTag') {
