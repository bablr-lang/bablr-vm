...
export const State = class BABLRState extends WeakStackFrame {
    ...
    for (let tagPath = TagPath.fromNode(node, 0); tagPath; tagPath = tagPath.nextSibling) {
      ...
      if (tag.type === Property && tag.value.reference.name) {
        ...
        let firstRef = isShift
          ? tagPath.siblingAt(tagPath.childrenIndex - refTag.value.index * 3 - 2).tag
          : refTag;
        if (firstRef.type !== ReferenceTag) throw new Error();
        ...
        if (isShift) {
          segment.shiftIndex = refTag.value.height || 0;
        }

        let expr = node.get(segment);
        newAgast.vm.next(
          buildChild(Property, buildProperty(firstRef.value, bindingTag.value, expr.node)),
        );
      } else {
        ...
        if (tag.type === Property) {
          ...
          let pathDesc = buildFullPathSegment(
            ...
            isArray
              ...
              : null,
            isShift ? refSibling.value.index + 1 : null,
          );
  ...
  reject(finishedMatch, options) {
    ...
    let shallower = finishedMatch.coveredBoundary.didShift
