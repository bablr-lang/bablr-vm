  ...
  while (!co.done) {
    ...
    switch (verb) {
      ...
      case 'reject':
        ...
        if (s.path.range[0] && !s.path.range[1]) {
          const { pathName, pathIsArray } = s.path.reference.value;
          const { resolver } = s.path.parent;

          if (!pathIsArray || !resolver.counters.has(pathName)) {
            yield buildCall('emit', s.path.reference);
