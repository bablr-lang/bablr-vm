...
export const State = class BABLRState extends WeakStackFrame {
  ...
  reject(next, abandon = false, bind = false) {
    const rejectedState = this;

    if (next) {
      const parentChildren = next.node.children;
      const ourChildren = this.node.children;
      const internalState = internalStates.get(next.node);

      // if (!abandon && next.node.type === rejectedState.node.type) {
      if (!abandon && next.node.type) {
        for (let i = sumtree.getSize(parentChildren); i < sumtree.getSize(ourChildren); i++) {
          ...
          if ([InitializerTag, GapTag].includes(tag.type)) {
            ...
            if (
              ...
              !hasOwn(next.node.properties, reference.value.name) &&
        ...
        let lastParentTagPath = TagPath.fromNode(next.node, -1);
        if (lastParentTagPath?.tag.type === ReferenceTag) {
          ...
          next.referencePath = null;
          next.resultPath = TagPath.fromNode(next.node, -1);
        }
      }
    }
    ...
    if (!next) throw new Error('rejected root state');
