import { agast as createAgast } from '@bablr/agast-vm';
...
import { getShifted, Path, TagPath } from '@bablr/agast-helpers/path';
  ...
  branch() {
    ...
    for (let tagPath = TagPath.fromNode(node, 0); tagPath; tagPath = tagPath.nextSibling) {
      ...
      if (tag.type === GapTag) {
        ...
        let path = isArray ? [name, btree.getSize(newAgast.state.node.properties[name])] : name;

        let expr = getShifted(ref.value.index || 0, path, node);
        newAgast.vm.next(buildEmbeddedNode(expr));
      } else {
    ...
    if (parent.depths.path === accepted.depths.path) {
      ...
      for (
        ...
        if (tag.type === GapTag) {
          ...
          parent.agast.vm.next(buildEmbeddedNode(expr));
        } else {
      ...
      if (!abandon && shallower.node.type) {
        ...
        for (let i = sumtree.getSize(parentChildren); i < sumtree.getSize(ourChildren); i++) {
          ...
          if ([InitializerTag, GapTag].includes(tag.type)) {
            ...
            let lastParentTag = TagPath.fromNode(shallower.node, -1).tag.type;
            ...
            if (
              ...
              if (lastParentTag !== ReferenceTag) {
                shallower.agast.vm.next(reference);
              }
        ...
        if (lastParentTagPath?.tag.type === ReferenceTag) {
          ...
          if (bind) {
            ...
            if (!ref.value.flags.expression) {
              shallower.agast.vm.next(buildInitializerTag(ref.value.isArray));
            }
          }
