import { agast as createAgast, NodeFacade, TagPathFacade as TagPath } from '@bablr/agast-vm';
...
import { buildNullNode, wrapperIsFull } from '@bablr/agast-helpers/path';
...
export const State = class BABLRState extends WeakStackFrame {
  ...
  branch() {
    ...
    if (node.tags.children) {
      let property = buildProperty(fragRefTag.value, null, node.tags.children);
      newAgast.vm.next(
        buildChild(
          PropertyWrapper,
          buildPropertyWrapper(
  ...
  reject(finishedMatch, options) {
    ...
    let abandon =
      ...
      !!finishedMatch.cover || // TODO: isCoverBoundary?
            ...
            for (let wrappedTag of tag.value.tags) {
              ...
              if ([InitializerTag, GapTag].includes(wrappedTag.type)) {
                ...
                if (
                  ...
                  !shallower.node.properties.has(referenceTag.value.name) &&
                  refTag !== ShiftTag
                ) {
