...
export class Context {
  ...
  getRangeFromMatch(match) {
    const { prevTags, pathsMap, ranges } = this;
    ...
    if (precedingToken === finalToken) {
      ...
      for (
        token = prevTags.get(finalToken);
        token !== precedingToken;
        token = prevTags.get(token)
      ) {
        if (token.type === EndNode) {
          const path = pathsMap.get(token);
          token = prevTags.get(ranges.get(path)[0]);
        }
  ...
  *ownTokensFor(range) {
    ...
    const { prevTags, paths, ranges } = this;
    const { 0: start, 1: end } = range;
    const prev = prevTags.get(start);

    for (let token = end; token !== prev; token = prevTags.get(token)) {
      if (token.type === EndNode) {
